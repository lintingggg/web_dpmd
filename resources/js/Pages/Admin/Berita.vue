...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
            <div>
                ...
                <p class="text-[14px] font-medium text-[#5b6b85]">Kelola publikasi berita, artikel, dan dokumentasi kegiatan dinas.</p>
            </div>
        ...
        <div class="bg-white rounded-[24px] shadow-[0_4px_20px_rgba(16,57,115,0.06)] border border-[#dbe6f7] overflow-hidden">
            ...
            <div class="p-6 md:p-8 border-b border-[#dbe6f7] flex flex-col xl:flex-row gap-6 justify-between items-start xl:items-center bg-[#f5f8fd]">
                ...
                <div class="relative w-full xl:w-80">
                    ...
                    <input v-model="search" type="text" placeholder="Cari judul berita..." class="w-full bg-white border border-[#dbe6f7] text-[#103973] text-[14px] rounded-xl pl-11 pr-4 py-2.5 focus:ring-[#1356a0] focus:border-[#1356a0]" />
                </div>
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full xl:w-auto overflow-x-auto pb-2 sm:pb-0">
                    <div class="flex items-center gap-3">
                        <span class="text-[11px] font-bold text-[#7488a8] uppercase tracking-[1px] whitespace-nowrap">Tag:</span>
                        <select v-model="tagFilter" class="bg-white border border-[#dbe6f7] text-[#103973] text-[13px] font-bold rounded-lg px-3 py-1.5 focus:ring-0 focus:border-[#c7dafa]">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse min-w-[900px]">
                    <thead>
                        <tr>
                            <th class="py-4 px-6 md:px-8 text-[11px] font-bold text-[#5b6b85] uppercase tracking-[2.75px] border-b border-[#dbe6f7]">Judul Berita</th>
                            <th class="py-4 px-6 text-[11px] font-bold text-[#5b6b85] uppercase tracking-[2.75px] border-b border-[#dbe6f7] w-48">Tags</th>
                            <th class="py-4 px-6 text-[11px] font-bold text-[#5b6b85] uppercase tracking-[2.75px] border-b border-[#dbe6f7] w-48">Penulis / Tgl</th>
                            <th class="py-4 px-6 text-[11px] font-bold text-[#5b6b85] uppercase tracking-[2.75px] border-b border-[#dbe6f7] w-32">Status</th>
                            <th class="py-4 px-6 md:px-8 text-[11px] font-bold text-[#5b6b85] uppercase tracking-[2.75px] border-b border-[#dbe6f7] text-right w-32">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="text-[14px] font-medium divide-y divide-[#dbe6f7]">
                        <tr v-for="item in berita.data" :key="item.id" class="hover:bg-[#f5f8fd]/50 transition-colors group">
                            ...
                            <td class="py-4 px-6 md:px-8">
                                <div class="flex items-center gap-4">
                                    <div class="w-20 h-14 rounded-lg bg-[#eaf1fb] flex items-center justify-center flex-shrink-0 border border-[#dbe6f7] overflow-hidden">
                                        ...
                                        <span v-else class="material-symbols-outlined text-[#7488a8] text-[24px]">image_not_supported</span>
                                    </div>
                                    <p @click="openDetailModal(item)" class="font-bold text-[#103973] leading-tight group-hover:text-[#1356a0] transition-colors cursor-pointer line-clamp-2">
                                        {{ item.judul }}
                                    </p>
                                </div>
                            </td>
                            ...
                            <td class="py-4 px-6">
                                <div class="flex flex-wrap gap-1.5 items-start">
                                    <span v-for="tag in item.tags" :key="tag" class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold tracking-[0.5px] uppercase bg-[#eaf1fb] text-[#1356a0]">
                                        {{ tag }}
                                    </span>
                                </div>
                            </td>
                            ...
                            <!-- Penulis & Tgl -->
                            <td class="py-4 px-6">
                                <p class="font-bold text-[#103973] mb-0.5">{{ item.penulis }}</p>
                                <p class="text-[12px] text-[#5b6b85]">{{ item.is_published ? formatDate(item.published_at) : 'Draft' }}</p>
                            </td>
                            ...
                            <td class="py-4 px-6">
                                ...
                                <span v-else class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-[#eaf1fb] text-[#5b6b85] border border-[#dbe6f7]">
                            ...
                            <td class="py-4 px-6 md:px-8 text-right">
                                <div class="flex items-center justify-end gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                                    <button @click="openModal(item)" class="w-8 h-8 rounded-full hover:bg-[#dbe6f7] text-[#5b6b85] flex items-center justify-center transition-colors" title="Edit">
                        ...
                        <tr v-if="berita.data.length === 0">
                            <td colspan="5" class="py-12 text-center text-[#5b6b85]">
            ...
            <div class="p-6 md:px-8 border-t border-[#dbe6f7] flex flex-col sm:flex-row items-center justify-between gap-4 bg-[#f5f8fd]">
                <p class="text-[13px] font-medium text-[#5b6b85]">
                    Menampilkan <span class="font-bold text-[#103973]">{{ berita.from || 0 }}-{{ berita.to || 0 }}</span> dari <span class="font-bold text-[#103973]">{{ berita.total }}</span> berita
                </p>
    ...
    <Modal :show="isModalOpen" @close="closeModal" maxWidth="2xl">
        <div class="p-6 md:p-8">
            <div class="flex items-center justify-between mb-6 pb-6 border-b border-[#dbe6f7]">
                <div>
                    <h3 class="text-[24px] font-bold text-[#103973] leading-tight">{{ isEditing ? 'Edit Berita' : 'Tambah Berita Baru' }}</h3>
                    <p class="text-[14px] text-[#5b6b85] mt-1">Isi formulir di bawah ini dengan detail berita.</p>
                </div>
                <button @click="closeModal" class="text-[#7488a8] hover:text-[#103973] transition-colors">
            ...
            <form @submit.prevent="submitForm" class="space-y-6">
                <!-- Judul -->
                <div>
                    <label class="block text-[14px] font-bold text-[#1356a0] mb-2">Judul Berita <span class="text-red-500">*</span></label>
                    <input v-model="form.judul" type="text" required class="w-full bg-[#f5f8fd] border border-[#dbe6f7] text-[#103973] text-[14px] rounded-lg px-4 py-2.5 focus:ring-[#1356a0] focus:border-[#1356a0]" placeholder="Masukkan judul..." />
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Thumbnail -->
                    <div>
                        <label class="block text-[14px] font-bold text-[#1356a0] mb-2">Thumbnail (Opsional)</label>
                        <div class="relative w-full border-2 border-dashed border-[#c7dafa] bg-[#f5f8fd] rounded-xl p-4 flex flex-col items-center justify-center text-center hover:bg-[#eaf1fb] transition-colors min-h-[120px]">
                            <span class="material-symbols-outlined text-[24px] text-[#5b6b85] mb-1">image</span>
                            <p class="text-[12px] font-medium text-[#1356a0] mb-1">
                    ...
                    <!-- Tags -->
                    <div>
                        <label class="block text-[14px] font-bold text-[#1356a0] mb-2">Tags</label>
                        <div class="w-full bg-[#f5f8fd] border border-[#dbe6f7] rounded-lg px-3 py-2 flex flex-wrap gap-2 focus-within:ring-1 focus-within:ring-[#1356a0] focus-within:border-[#1356a0]">
                            <span v-for="(tag, index) in form.tags" :key="index" class="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-[#dbe6f7] text-[#103973] text-[12px] font-semibold">
                                {{ tag }}
                        ...
                        <p class="text-xs text-[#7488a8] mt-1">Tekan Enter untuk menambahkan tag.</p>
                ...
                <!-- Konten Rich Text -->
                <div>
                    <label class="block text-[14px] font-bold text-[#1356a0] mb-2">Konten Berita <span class="text-red-500">*</span></label>
                ...
                <div class="pt-2">
                    <label class="flex items-center gap-3 cursor-pointer group w-max">
                        ...
                        <div class="flex flex-col">
                            <span class="text-[14px] font-bold text-[#103973] leading-none mb-1">Publikasikan Berita</span>
                            <span class="text-[12px] text-[#5b6b85] leading-none">Berita akan tampil di website publik</span>
                        </div>
                    </label>
                </div>
                ...
                <div class="pt-6 border-t border-[#dbe6f7] flex justify-end gap-3">
                    <button type="button" @click="closeModal" class="px-6 py-2.5 rounded-full border border-[#c7dafa] bg-white text-[#1356a0] font-bold text-[14px] hover:bg-[#eaf1fb] transition-all active:scale-95">
                        Batal
                    </button>
    ...
    <Modal :show="isDetailOpen" @close="isDetailOpen = false" maxWidth="2xl">
        <div class="relative bg-white overflow-hidden" v-if="detailItem">
            ...
            <div class="w-full h-64 bg-[#eaf1fb] relative">
                ...
                <div v-else class="w-full h-full flex flex-col items-center justify-center text-[#7488a8]">
            ...
            <div class="p-6 md:p-8">
                ...
                <div class="flex flex-wrap items-center gap-2 mb-4">
                    ...
                    <span v-for="tag in detailItem.tags" :key="tag" class="text-[12px] font-bold uppercase tracking-[0.5px] text-[#1356a0]">
                        #{{ tag }}
                    </span>
                </div>
                
                <!-- Title -->
                <h2 class="text-[28px] md:text-[32px] font-bold text-[#103973] leading-tight mb-4">
                    {{ detailItem.judul }}
                </h2>
                
                <!-- Author & Date -->
                <div class="flex items-center gap-4 text-[13px] text-[#5b6b85] mb-8 pb-6 border-b border-[#dbe6f7]">
                    <div class="flex items-center gap-1.5">
                        <span class="material-symbols-outlined text-[16px]">edit_document</span>
                        <span class="font-bold text-[#1356a0]">{{ detailItem.penulis }}</span>
                    </div>
                    <div class="w-1 h-1 rounded-full bg-[#c7dafa]"></div>
                    <div class="flex items-center gap-1.5">
                ...
                <!-- Content -->
                <div class="prose prose-sm md:prose-base prose-slate max-w-none prose-headings:font-bold prose-a:text-[#1356a0] prose-img:rounded-xl" v-html="detailItem.konten"></div>
                
                <div class="mt-10 pt-6 border-t border-[#dbe6f7] flex justify-end">
                    <button @click="isDetailOpen = false" class="px-6 py-2.5 rounded-full border border-[#c7dafa] bg-white text-[#1356a0] font-bold text-[14px] hover:bg-[#eaf1fb] transition-all active:scale-95">
                        Tutup
                    </button>
                </div>
            </div>
    ...
    <Modal :show="isDeleteModalOpen" @close="isDeleteModalOpen = false" maxWidth="sm">
        <div class="p-6 text-center">
            ...
            <h3 class="text-[20px] font-bold text-[#103973] mb-2">Hapus Berita?</h3>
            <p class="text-[14px] text-[#5b6b85] mb-6">
                Apakah Anda yakin ingin menghapus berita <span class="font-bold text-[#103973]">"{{ itemToDelete?.judul }}"</span>? Tindakan ini tidak dapat dibatalkan.
            </p>
            <div class="flex justify-center gap-3">
                <button @click="isDeleteModalOpen = false" class="px-5 py-2.5 rounded-full border border-[#c7dafa] bg-white text-[#1356a0] font-bold text-[14px] hover:bg-[#eaf1fb] transition-all active:scale-95">
                    Batal
                </button>
